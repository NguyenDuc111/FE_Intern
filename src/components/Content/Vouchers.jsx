          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {vouchers.map((voucher) => (
                ...
                >
                  <h3 className="text-lg font-semibold text-white mb-2 sm:text-xl">{voucher.name}</h3>
                  <p className="text-white text-sm sm:text-base">📌 Giảm: {voucher.discount}{voucher.discount <= 100 ? "%" : " VND"}</p>
                  <p className="text-white text-sm sm:text-base mt-1">🎯 Điểm cần: {voucher.pointsRequired}</p>
                  <p className="text-white text-sm sm:text-base mt-1">🔄 Lượt đổi còn lại: {voucher.redemptionsRemaining > 0 ? `Còn ${voucher.redemptionsRemaining} lượt đổi` : "Hết lượt đổi"}</p>

                  <button
                    onClick={() => handleRedeem(voucher.id)}
                    disabled={voucher.redemptionsRemaining === 0 || redeeming[voucher.id]}
                    className={`mt-4 px-4 py-2 rounded-lg font-semibold text-white flex items-center justify-center min-w-[150px] ${voucher.redemptionsRemaining === 0 || redeeming[voucher.id] ? "bg-gray-400 cursor-not-allowed" : "bg-yellow-600 hover:bg-yellow-700"} transition-all duration-300`}
                  >
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {redeemedVouchers.map((voucher) => (
                ...
                >
                  <h3 className="text-lg font-semibold text-white mb-2 sm:text-xl">{voucher.name}</h3>
                  <p className="text-white text-sm sm:text-base">📌 Giảm: {voucher.discount}{voucher.isPercentage ? "%" : " VND"}</p>
                  <p className="text-white text-sm sm:text-base mt-1">🧾 Mã Voucher: {voucher.voucherCode}</p>
                  <p className="text-white text-sm sm:text-base mt-1">⏰ Hết hạn: {new Date(voucher.expiryDate).toLocaleDateString()}</p>
                  <p className="text-white text-sm sm:text-base mt-1">📍 Trạng thái: {getStatusText(voucher.status)}</p>
                </div>
              ))}
            </div>
          )}
